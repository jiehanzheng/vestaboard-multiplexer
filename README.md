...
The optional EMV problem indicator replaces the position digits with a red tile and `MFO`. In Water heater settings, select a problem entity and choose on/off or the heater’s “Missed flow off active” / “Missed flow off not active” states. Leave the entity empty to disable it. Missing, restored, or unavailable readings keep the position display and show a diagnostic.
