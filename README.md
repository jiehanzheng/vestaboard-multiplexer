...
The optional EMV problem indicator replaces the position digits with a red tile and `MFO`. In Water heater settings, select a problem entity and enter the exact, case-sensitive value that activates it (for example, `on` or `Missed flow off active`). Leave the entity empty to disable it. Missing, restored, or unavailable readings keep the position display and show a diagnostic.
