...
The optional EMV problem indicator replaces the position digits with a red tile and `MFO`. In Water heater settings, select a problem entity and enter the exact, case-sensitive value that activates it (for example, `on` or `Missed flow off active`). Leave the entity empty to disable it. Missing, restored, or unavailable readings show a diagnostic; the stale timeout limits how long the position display remains visible.

Each plugin has a **Stale readings** setting (15 minutes by default, or never expire). Codex ages from its last successful collection; Water Heater ages from when an input becomes unavailable or HA disconnects. Stale elements show `N/A`; constants and healthy, unchanged HA entities do not expire. Recovery is automatic. Expiry uses the minute refresh and normal board delivery limits; it does not change pause automation. Retained readings are not persisted across restarts.
