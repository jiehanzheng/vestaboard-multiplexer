...
/** Durable manual and platform pause state; failed writes remain paused. */
export class PauseStore {
  ...
  static async open(directory: string): Promise<PauseStore> {
    ...
    try {
      ...
      if (typeof value.manualPause !== "boolean" || (value.haPause !== undefined && typeof value.haPause !== "boolean") ||
          (value.haBinding !== undefined && typeof value.haBinding !== "string")) {
        throw new Error("Invalid pause state");
      }
      // Older pause files contain only manualPause; a missing HA field is inactive.
      store.state = { manualPause: value.manualPause, haPause: value.haPause === true, haBinding: value.haBinding as string | undefined };
