...
test("HTTP actions validate JSON/origin and SSE reconnect receives current snapshot", async () => {
  ...
  const config = () => ({ config: { ...DEFAULT_APP_CONFIG, transport: { ...DEFAULT_APP_CONFIG.transport, token: "server-only" }, ha: { ...DEFAULT_APP_CONFIG.ha, token: "ha-server-only" } }, locked: [], hasSecrets: { token: true, localApiKey: false, haToken: true } });
